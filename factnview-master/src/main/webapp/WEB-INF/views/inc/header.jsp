...
<header id="header">
	<nav class="container">
	    <div class="nav-wrapper">
	      <a href="../main/index" class="brand-logo logo font"><img src="../resource/images/fnvlogo2.jpg"/>
	      <div class="mlogo nanum">Fact & View</div></a>
	      ...
	      <ul class="right hide-on-med-and-down">
	        <c:choose>
				<c:when test="${not empty noticeboard  }">
					<li class="active nanum"><a class="board nanumgothic"
			...
			<c:choose>
				<c:when test="${not empty freeboard  }">
					<li class="active nanum"><a class="board nanumgothic"
						href="../freeboard/freeboard">Fact</a></li>
				</c:when>
				<c:otherwise>
					<li><a class="board nanum" href="../freeboard/freeboard">Fact</a></li>
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				<c:when test="${not empty siteboard  }">
					<li class="active nanum"><a class="board nanumgothic"
						href="../siteboard/siteboard">View</a></li>
				</c:when>
				<c:otherwise>
					<li><a class="board" href="../siteboard/siteboard">View</a></li>
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				<c:when test="${not empty requestboard  }">
					<li class="active"><a class="board nanumgothic"
			...
			<security:authorize access="isAnonymous()">
						...
						<a class="login font pin dropdown-button" href='#'
							data-activates='dropdown1'> <security:authentication
								property="name" /> <i class="material-icons right">arrow_drop_down</i>
						</a>

						<!-- Dropdown Structure -->
						<ul id='dropdown1' class='dropdown-content'>

							<li><a href="#modal110" onclick="userCheck();"><i
									class="material-icons md-36">face</i>Profile</a></li>

							<li><a href="../user/mypage?memberId=${loginID}"><i
									class="material-icons">pages</i>MyPage</a></li>

							<security:authorize access="hasRole('ROLE_ADMIN')">
								<li><a href="../admin/admin"><i class="material-icons">pages</i>AdminPage</a></li>
							</security:authorize>
	  ...
	  <!--  <li><a class="board " href="../freeboard/freeboard">chat</a></li> -->
	      ...
	      <ul class="side-nav" id="mobile-demo">
	      	<security:authorize access="isAnonymous()">
				...
				<a class="login font personal input" href="#modal11">Join 
					<i class="tiny material-icons">input</i>
				</a>
			</security:authorize>
			<security:authorize access="isAuthenticated()">
<%-- 				<a class="login font personal pin" href="${root}/user/mypage"> 
					<security:authentication property="name" /><i class="tiny material-icons">person_pin</i> </a> --%>
				<a class="login font personal pin" href="#" data-activates="profile-dropdown"> 
				<security:authentication property="name" /><i class="tiny material-icons">person_pin</i>
				</a>
			...
			<c:choose>
				<c:when test="${not empty freeboard  }">
					<li class="active"><a class="board " href="../freeboard/freeboard">Fact</a></li>
				</c:when>
				<c:otherwise>
					<li><a class="board " href="../freeboard/freeboard">Fact</a></li>
				</c:otherwise>
			</c:choose>
			
			<c:choose>
				<c:when test="${not empty siteboard  }">
					<li class="active"><a class="board " href="../siteboard/siteboard">View</a></li>
				</c:when>
				<c:otherwise>
					<li><a class="board " href="../siteboard/siteboard">View</a></li>
				</c:otherwise>
  ...
  <div id="modal10" class="modal">
    <form id="myForm20" action="../j_spring_security_check" method="post">
...
<%--  원래 <form id="myForm3" action="${root}/joinus/singIn" method="post"> --%>
    <form id="myForm3" action="../joinus/singIn" method="post">
            <div class="modal-content">
                <div class="row">
			         <div class="input-field col s12 center">
			            <h4>Register</h4>
		...
		<form id="profileForm" action="${root }/joinus/profileUpdate"method="post" enctype="multipart/form-data" >
			<div class="info_top">
				<input id="profileImgbtn" name="file" type="file" style="display: none">
				 ...
				 <img id="profileImg" alt="" src="../resource/images/avatar.png">
				<div>
					<span><security:authentication property="name" /></span>
				</div>
				<div class="introduction">
					<textarea placeholder="사용할 닉네임을 입력해주세요." class="pr" name="user[introduction]" id="user_introduction"></textarea>
				</div>
				<div class="btn_area">
...
<script type="text/javascript">
      $(document).ready(function(){
          ...
          $("#btn2").click(function(){
                ...
                $.post("../joinus/login", {"j_username":j_name.val(),"j_password":j_pwd.val()}, function(d) {
      ...
      function userCheck(){
    	  console.log('${loginID}');
    	  var userName = '${loginID}';
    	  $.get("../joinus/getUser",{"email":userName} ,function(d){
    		  
    		  var obj = JSON.parse(d);
    		  
   /*  		 alert(obj.profile);  */
   			  if(obj.profile != ''){
   				profileImg.src = "../resource/profile/"+obj.profile;  
   			  }
   			  else{
...
/* 				$.post("${root}/joinus/isSingIn", email, function(d) {
		10.16 작동안되서 아래로 대체함
					 */
				$.post("../joinus/isSingIn", email, function(d) {
					if(d == '1'){
						alert("중복된 ID입니다");	
					}
					else{
