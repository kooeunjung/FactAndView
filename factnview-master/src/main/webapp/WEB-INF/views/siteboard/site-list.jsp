...
<link href="//netdna.bootstrapcdn.com/font-awesome/3.2.1/css/font-awesome.css" rel="stylesheet">
			...
			<div class="site-list">
				<div class="collection mvaside">
					<a
						href="siteboard?p=1&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}"
						class="collection-item"><span class="new badge">${sitenew.news }</span>최신순</a>
					<a
						href="siteboardlike?p=1&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}"
						class="collection-item"><span class="new badge">${sitenewL.news }</span>베스트
						사이트</a> <a
						href="siteboardhit?p=1&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}"
...
<div style="display: none"><fmt:formatDate value="${now}" pattern="yyyy-MM-dd" var="today" /> </div>
						<li class="list-item flex">
							<a href="site-detail?c=${v.id}" class="url">
								<div class="index">
									<p>${v.id }</p>
								</div>
								<div class="title">
								...
								<div  class="like-warpper">
									...
									<div>
										<label class="like-s"></label>
									</div>
								</div>
								<div class="url-warpper">
									<img src="http://api.thumbalizr.com/?url=http://${v.url}" />
								</div>
							</a>
						</li>
...
<ul class="pagination center">
	<li class="disabled"><c:if test="${((listPerFive-1)*5 + 5) > 1 }">
			<a
				href="?p=${(listPerFive-1)*5 + 5 }&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}"><i
		...
		<c:choose>
			<c:when test="${i eq param.p or empty param.p and i eq '1'}">
				<li class="waves-effect  active"><a
					href="?p=${i }&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}">${i}</a></li>
			</c:when>
			<c:otherwise>
	...
	<li class="waves-effect"><c:if test="${cnt > ((listPerFive+1)*5 + 1) }">
			<a
				href="?p=${(listPerFive+1)*5 + 1 }&query=${query}&bigCa=${param.bigCa}&smallCa=${param.smallCa}"><i
